...
int encrypt(unsigned char *plaintext, int plaintext_len, unsigned char *key, unsigned char *iv, unsigned char *ciphertext) {
  ...
  if(1 != EVP_EncryptInit_ex(ctx, EVP_camellia_128_cfb(), NULL, key, iv)) handleErrors();
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    cout << "Usage: " << argv[0] << " <key> <iv> <input_file>" << endl;
    return 1;
  }
  ...
  //get the iv from command line argument
  unsigned char iv[16];
  std::string iv_arg(argv[2]);
  for (std::string::size_type i = 0; i < iv_arg.size(); i += 2) {
    iv[i/2] = std::stoi(iv_arg.substr(i, 2), nullptr, 16);
  }
