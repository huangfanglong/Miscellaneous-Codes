...
void init_vals(list<string> &names, args &var)
{
	cout << "Number of people? " << var.N << endl;
	cout << "Index for elimination? " << var.M << endl;
	cout << "Index for printing? " << var.K << endl;
	
	names.resize(var.N);
	for(auto i : names)
	{
		generate(names.begin(), names.end(), SEQ(var.N));
	}
